...
function Clock() {
    ...
    const checkTwoNumber = (num: number) => {
        return String(num).length < 2 ? '0' + num : String(num);
    }

    const stringTime = `${checkTwoNumber(date.getHours())}:${checkTwoNumber(date.getMinutes())}:${checkTwoNumber(date.getSeconds())}` ||
        <br/> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
    const stringDate = `${checkTwoNumber(date.getDate())}.${checkTwoNumber(date.getMonth() + 1)}.${checkTwoNumber(date.getFullYear())}` ||
